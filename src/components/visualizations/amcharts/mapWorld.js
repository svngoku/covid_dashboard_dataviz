...
const MapWorldCovid = () => {
    ...
    useEffect(() => {
        ...
        async function getCovidData() {
            ...
            let countriesCode = Countries.map((c) => c.CountryCode)
            let countriesDeaths = Countries.map((c) => c.TotalDeaths)
            // setData([
            //     { "id":countriesCode , "name":countriesName, "value": countriesDeaths, "color": chart.colors.getIndex(1) }
            // ])
        }
        ...
        title.text = "[bold font-size: 20]Covid-19 Data[/]\n";
